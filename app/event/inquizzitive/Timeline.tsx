  ...
  return (
    ...
    >
      ...
      <div className="maxWidthForSections  flex flex-col items-center">
        ...
        <div className={`flex flex-col items-center  ${`w-`+1.208*imgWidth +`vw `}`}>
          <div className="grid grid-cols-2 sm:gap-x-4 gap-x-2 mb-1 px-2">
            {faqDetails.map((event,index) => {
              return (
                <div key={event.day + index} className="bg-[#E9A128] sm:min-h-[130px]   sm:py-6 sm:px-4 p-2 py-4 rounded-md text-center flex flex-col justify-start text-darkBlue">
                  <h1 className="sm:text-xl text-lg">{event.day}</h1>
                  <div className="w-[40%] mx-auto h-1 my-1 rounded-full bg-darkBlue"></div>
                  <p className="text-center sm:text-base text-sm">{event.task}</p>
                </div>
              );
            })}
          </div>
